...
class StorageQuerySet(models.QuerySet):
    ...
    def available_boxes(self):
        return self.annotate(
            empty_boxes=Count(
                "boxes", filter=Q(boxes__is_available=True), distinct=True
            ),
...
class Storage(models.Model):
    ...
    temperature = models.SmallIntegerField(
        verbose_name="Температура на складе", blank=True, null=True
    )
    ...
    route_description = models.TextField(
        verbose_name="Проезд до склада", blank=True
    )
    special_property = models.CharField(
...
class Box(models.Model):
    ...
    floor = models.SmallIntegerField(
        verbose_name="Этаж", blank=True, null=True
    )
...
class Order(models.Model):
    ...
    start_order = models.DateField(
        verbose_name="Начало аренды", default=datetime.datetime.now
    )
